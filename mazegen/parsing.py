...
ConfigValue = Union[int, Tuple[int, int], bool, str]


def parsing_line(line: str) -> tuple[str, ConfigValue]:
    line = line.strip()
    if "=" not in line:
    ...
    if key in ("WIDTH", "HEIGHT"):
        ...
        if value == "":
            raise ValueError(
                "invalid input seed can just be "
...
def validate_config(config: dict[str, ConfigValue]) -> None:
    ...
    if not (0 <= x_entry < config["WIDTH"] and
            0 <= y_entry < config["HEIGHT"]):
...
def read_file(path: str = "config.txt") -> Dict[str, ConfigValue]:
    config = {}
