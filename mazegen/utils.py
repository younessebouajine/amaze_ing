...
def _42cells(maze_width: int, maze_height: int) -> List[Tuple]:
    cells = [
...
def get_neighbors(grid: List[List[Cell]],
                  current_cell: Cell, width: int,
...
def get_open_neighbors(grid: List[List[Cell]],
                       current_cell: Cell, width: int,
    ...
    for direction, x, y in directions:
        if 0 <= current_cell.x < width and 0 <= current_cell.y < height:
            ...
            print(
                ...
                next_cell.isvisited,
                direction,
                this_cell.walls[direction],
                not next_cell.isvisited and not this_cell.walls[direction],
            )
...
def get_path(path: dict, grid: list,
             output_file, print_to_file, color="\033[97m"):
    # RESET = "\033[0m"
    solution_key = None
